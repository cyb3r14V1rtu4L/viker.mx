...
class RouteCollectionBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFlushOrdering()
    {
        ...
        // 4) Add another route
        $routes->add('/enterprise', 'AppBundle:Admin:enterprise', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/enterprise', 'AppBundle:Admin:enterprise', 'admin_dashboard');
    ...
    public function providePrefixTests()
    {
        ...
        $tests[] = array('/{enterprise}', '/foo', '/{enterprise}/foo');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        $adminRoutes->add('/enterprise', 'AdminController::dashboardAction', 'admin_dashboard');

        // embedded collection under /enterprise
        $adminBlogRoutes = $routes->createBuilder();
        $adminBlogRoutes->add('/new', 'BlogController::newAction', 'admin_blog_new');
        // mount into enterprise, but before the parent collection has been mounted
        $adminRoutes->mount('/blog', $adminBlogRoutes);

        // now mount the /enterprise routes, above should all still be /blog/enterprise
        $routes->mount('/enterprise', $adminRoutes);
        ...
        $loader
            ...
            ->method('load')
            ->will($this->returnValue($importedCollection));
        // import this from the /enterprise route builder
        $adminRoutes->import('enterprise.yml', '/imported');

        $collection = $routes->build();
        $this->assertEquals('/enterprise/enterprise', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/enterprise/users', $collection->get('admin_users')->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/enterprise/blog/new', $collection->get('admin_blog_new')->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/enterprise/stats/sales', $collection->get('admin_stats_sales')->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
        $this->assertEquals('/enterprise/imported/foo', $collection->get('imported_route')->getPath(), 'Normal RouteCollections are also prefixed properly');
    }
    ...
    public function testAutomaticRouteNamesDoNotConflict()
    {
        ...
        $adminRoutes = $routes->createBuilder();
        // route 1
        $adminRoutes->add('/enterprise', '');

        $accountRoutes = $routes->createBuilder();
        // route 2
        $accountRoutes->add('/enterprise', '')
            ->setMethods(array('GET'));
        // route 3
        $accountRoutes->add('/enterprise', '')
            ->setMethods(array('POST'));
